...
import { prepareRender, cameras, drawCommands, entitiesFromSolids } from '@jscad/regl-renderer'
...
export function JSCADViewer({ solids }: { solids: any }) {
  ...
  useEffect(() => {
    ...
    function getContextAndContextType(canvas: HTMLCanvasElement, contextType: string) {
      const context = canvas.getContext(contextType);
      return context ? { context, contextType } : null;
    }

    const contextAndContextTypeResult = getContextAndContextType(canvas, 'webgl2')
    ...
    if (webglContextType === 'webgl') {
      ...
      if ((webglContext as any)?.getExtension('OES_element_index_uint')) {
        if (viewerOptions.glOptions) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (viewerOptions.glOptions as any).optionalExtensions = ['oes_element_index_uint']
        }
      }
    }
    ...
    cameras.perspective.setProjection(refViewerOptions.current.camera, refViewerOptions.current.camera, {
      width: refCanvas.current?.width ?? 0,
      height: refCanvas.current?.height ?? 0
    })
    cameras.perspective.update(refViewerOptions.current.camera, refViewerOptions.current.camera);
  ...
  const animationFrameCallback = useCallback(() => {
    if (viewerReady === true && refNeedsRerender.current === true && refRender.current !== undefined) {
